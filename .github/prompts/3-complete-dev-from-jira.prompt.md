...
- Deliver production-ready code changes (testing will be handled in separate phases)
...
- **DO NOT create unit tests** - testing is handled in separate phases (Phase 4-6)
...
5. **Code Implementation:** Develop features according to specifications
   ...
   - **DO NOT create unit test files** - testing handled in separate phases
6. **Code Review:** Self-review for quality, standards, and completeness
7. **Pre-Commit Cleanup:** Stop any running development servers before git operations
   - Stop dev servers (npm run dev, etc.) if running
   - Remove any build artifacts (dist/, build/) from staging
8. **Commit Changes:** Create descriptive commits referencing Jira story
9. **Initial Push:** Push feature branch to remote for backup and collaboration
...
- **Implementation Code:** Complete feature implementation
  ...
  - **Excludes:** Unit test files (handled in testing phases)
...
- **Testing:** Unit and E2E tests will be created in separate testing phases
...
- **Pre-Commit:** Stop development servers and exclude build artifacts from commits
...
- Implementation is ready for code review and testing phases (Phase 4-6)
- Changes are properly documented and traceable to requirements
- Development servers are properly stopped before git operations
- Only source code changes are committed (no build artifacts or test files)
...
**NOTE:** This phase focuses solely on feature implementation. Unit tests, integration tests, and E2E tests will be created in separate testing phases (Phase 4-6) to maintain clean separation of concerns in the SDLC workflow.
